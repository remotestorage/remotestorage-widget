...
class Widget {
	...
	constructor(remoteStorage: RemoteStorage, options: WidgetOptions = {}) {
		...
		this.autoCloseAfter = options.autoCloseAfter
	...
	setState(state: string): void {
		...
		const lastSelected = this.parentContainerEl?.querySelector(
			".rs-box.rs-selected",
		);
		if (lastSelected) {
		...
		const toSelect = this.parentContainerEl?.querySelector(
			`.rs-box.rs-box-${state}`,
		);
		if (toSelect) {
		...
		const currentStateClass =
			this.rsWidget.className.match(/rs-state-\S+/g)?.[0];
	...
	setupElements(): void {
		...
		this.rsBackdrop = this.parentContainerEl.querySelector(
		...
		this.rsConnected =
			this.parentContainerEl.querySelector(".rs-box-connected");
		...
		this.rsChooseRemoteStorageButton = this.parentContainerEl.querySelector(
			"button.rs-choose-rs",
		);
		this.rsChooseDropboxButton = this.parentContainerEl.querySelector(
			"button.rs-choose-dropbox",
		);
		this.rsChooseGoogleDriveButton = this.parentContainerEl.querySelector(
			"button.rs-choose-googledrive",
		);
		this.rsErrorBox = this.parentContainerEl.querySelector(
		...
		this.rsSignInForm =
			this.parentContainerEl.querySelector(".rs-sign-in-form");
		...
		this.rsDisconnectButton =
			this.parentContainerEl.querySelector(".rs-disconnect");
		...
		this.rsConnectedUser = this.parentContainerEl.querySelector(
	...
	setEventListeners(): void {
		this.rsSignInForm.addEventListener("submit", (e) => {
			e.preventDefault();
			const userAddress = this.parentContainerEl.querySelector(
				"input[name=rs-user-address]",
			).value;
	...
	setClickHandlers(): void {
		...
		this.rsErrorReconnectLink.addEventListener("click", () =>
			this.rs.reconnect(),
		);
		this.rsErrorDisconnectButton.addEventListener("click", () =>
			this.rs.disconnect(),
		);
	...
	open(): void {
		...
		const selected = this.parentContainerEl.querySelector(
			".rs-box.rs-selected",
		);
		if (selected) {
	...
	handleDiscoveryError(error) {
		const msgContainer =
			this.parentContainerEl.querySelector(".rs-sign-in-error");
